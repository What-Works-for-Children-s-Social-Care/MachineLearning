...
def _ax_title(ax, title, subtitle):
	...
	title = '\n'.join(wrap(title, 60))
	ax.set_title(title, fontname="Arial", color = '#4d4d51', fontsize=12, loc='left') # x=0.5, y=1.05
...
def _ax_labels(ax, xlabel, ylabel):
	...
	ylabel = '\n'.join(wrap(ylabel, 30))
	xlabel = '\n'.join(wrap(xlabel, 60))   
	ax.set_xlabel(xlabel, fontname="Arial", color = '#4d4d51', fontsize=12)
	ax.set_ylabel(ylabel, fontname="Arial", color = '#4d4d51', fontsize=12)    
...
def _ax_grid(ax, status):
	ax.yaxis.grid(status, linestyle='-', alpha=0.4,color="#d0dde1") 
	ax.xaxis.grid(False)
...
def _ax_boxplot(ax, ALE, cat, **kwargs):
	ax.boxplot(cat, ALE, color="#fd9b8c", **kwargs)

def _ax_hist(ax, x, **kwargs):
	sns.rugplot(x, ax=ax, alpha=0.2, color="#88d0d9")


def _first_order_quant_plot(ax, quantiles, ALE, **kwargs):
	#ax.plot(quantiles, ALE, **kwargs)
	ax.plot((quantiles[1:] + quantiles[:-1]) / 2, ALE, color="#ff7057", **kwargs)
...
def _first_order_ale_quant(predictor, train_set, feature, quantiles):
	...
	for i in range(1, len(quantiles)):
		...
		if len(subset) != 0:
			...
			ALE[i - 1] += (np.subtract(predictor(z_up)[:,1], predictor(z_low)[:,1], dtype=np.float32)).sum()/subset.shape[0]
	...
	for i in range(1, len(quantiles[0])):
		for j in range(1, len(quantiles[1])):
			...
			if len(subset) != 0:
				...
				ALE[i, j] += (predictor(z_up[1].values)[:,1] - predictor(z_up[0].values)[:,1] - (predictor(z_low[1].values)[:,1] - predictor(z_low[0].values)[:,1])).sum() / subset.shape[0]
...
def _first_order_ale_cat(predictor, train_set, feature, features_classes, feature_encoder=None):
	...
	for i in range(num_cat):
		...
		if len(subset) != 0:
			...
			z_low[feature] = quantiles[i - 1]
			z_up[feature] = quantiles[i]
			ALE[i] += (predictor(z_up.values)[:,1] - predictor(z_low.values)[:,1]).sum() / subset.shape[0]
...
def ale_plot(model, train_set, features, feature_name, outcome, txt, file_name, bins=10, monte_carlo=False, predictor=None, features_classes=None, **kwargs):
		...
		if monte_carlo:
			...
			for k, rep in enumerate(mc_replicates):
				...
				if features_classes is None:
					...
					_first_order_quant_plot(fig.gca(), quantiles, mc_ALE, alpha=0.06)

		if features_classes is None:
			ALE = _first_order_ale_quant(model.predict_proba if predictor is None else predictor, train_set, features[0], quantiles)
			_ax_labels(fig.gca(), "{}".format(feature_name), "ALE of {}".format(outcome))
			_ax_title(fig.gca(), "How {} influences the predicted probability".format(feature_name), "")
			_ax_grid(fig.gca(), True)
			_ax_hist(fig.gca(), train_set[features[0]])
			_first_order_quant_plot(fig.gca(), quantiles, ALE)
			#_ax_quantiles(fig.gca(), quantiles)
		else:
			_ax_boxplot(fig.gca(), quantiles, ALE, color="#fd9b8c")
	elif len(features) == 2:
		quantiles = [np.percentile(train_set[f], [1. / bins * i * 100 for i in range(0, bins + 1)]) for f in features]

		if features_classes is None:
			ALE = _second_order_ale_quant(model.predict_proba if predictor is None else predictor, train_set, features, quantiles)
	...
	fig.suptitle("ACCUMULATED LOCAL EFFECTS (ALE)", fontname="Arial", color = '#ff7057', fontsize=12, x=0.38, y=1.10)
	plt.figtext(0, -0.05, txt[0], wrap=True, fontname="Arial", color='#4d4d51', fontsize=12)
	plt.figtext(0, -0.1, txt[1], wrap=True, fontname="Arial", color='#4d4d51', fontsize=12)
	plt.figtext(0, -0.15, txt[2], wrap=True, fontname="Arial", color='#4d4d51', fontsize=12)
